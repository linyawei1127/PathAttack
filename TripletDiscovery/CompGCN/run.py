...
class Runner(object):
	...
	def load_data(self):
		...
		# for split in ['train', 'test', 'valid', 'target']:
		...
		#for split in ['test', 'valid', 'target']:
		...
		self.data_iter = {
			...
			# 'target_head':  get_data_loader(TestDataset,  'target_head',  self.p.batch_size),
			# 'target_tail':  get_data_loader(TestDataset,  'target_tail',  self.p.batch_size),
		}
	...
	def evaluate(self, split, epoch):
		...
		self.logger.info('[Epoch {} {}]: Hit@1 : {:.5}, Hit@5 : {:.5}, Hit@10 : {:.5}'.format(epoch, split, results['hits@1'], results['hits@5'], results['hits@10']))
		return results
	...
	def fit(self):
		...
		if self.p.restore:
			# save_path = "/home/nudt/linyawei/CompGCN/checkpoints/WN18RR_original"
			self.load_model(save_path)
			self.logger.info('Successfully Loaded previous model')
			# val_results = self.evaluate('valid', 0)
		# target_results = self.evaluate('target', 0)
		kill_cnt = 0
		...
		# target_results = self.evaluate('target', epoch)
...
if __name__ == '__main__':
	...
	if not args.restore: args.name = args.dataset + '_' + args.opn + '-' + time.strftime('%d_%m_%Y') + '_' + time.strftime('%H:%M:%S')
