...
#         o_real, o_img = torch.chunk(obj_emb, 2, dim=-1)
    ...
    def score_sr(self, sub, rel, sigmoid=False):
        """
        给定(subject, relation)，返回所有可能 object 的打分向量
        相当于 forward(sub, rel, mode='rhs')
        """
        # 调用 forward，mode='rhs' 对应 (s,r)->o
        return self.forward(sub, rel, mode='rhs', sigmoid=sigmoid)

    def score_or(self, obj, rel, sigmoid=False):
        """
        给定(object, relation)，返回所有可能 subject 的打分向量
        相当于 forward(obj, rel, mode='lhs')
        """
        # 调用 forward，mode='lhs' 对应 (o,r)->s
        return self.forward(obj, rel, mode='lhs', sigmoid=sigmoid)
