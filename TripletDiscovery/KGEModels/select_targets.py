...
def set_paths(args):
    ...
    model_path = './saved_models/processed_WN18RR_distmult_200_0.2_0.3_0.3.model'
...
if __name__ == '__main__':
    ...
    with torch.no_grad():
        target_path = './target_{0}_{1}_{2}.txt'.format(args.model, args.data, args.target_split)
        # generate ranks for test set 
        train_data = torch.from_numpy(train_data.astype('int64')).to(device)
        if args.add_reciprocals:
            num_rel= n_rel
        else:
            num_rel = 0
        ranks_lhs, ranks_rhs = get_ranking(model, train_data, num_rel, to_skip_eval, device, args.test_batch_size)
        ...
        elif args.target_split == 3:
            indices = np.asarray((ranks_lhs <= 40) & (ranks_rhs <= 40)).nonzero()
        else:
