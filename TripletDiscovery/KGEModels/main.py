...
class Main(object):
    ...
    def load_data(self):
        ...
        self.target_data = self._load_data(os.path.join(data_path, 'target.txt'))
    ...
    def load_model(self):
        # self.model_path = "/home/nudt/linyawei/AttributionAttack/KGEAttack/ConvE/saved_models/processed_FB15k-237_original_distmult_200_0.5_0.3_0.3.model"
    ...
    def evaluate(self, split, batch_size, epoch):
        ...
        with torch.no_grad():
            ...
            if split == 'test':
                ...
                results = evaluation(self.model, self.target_data, self.to_skip_eval, 
                                     self.eval_name, num_rel, split, batch_size, epoch, self.device)
            else:
                data_path = 'data/{0}'.format(self.args.data)
    ...
    def fit(self):
        ...
        train_losses = []
        best_valid_metric = 0
        patience_counter = 0  # 用于记录验证集性能未提升的轮次数
        patience = 10 # 默认早停耐心为 10
        ...
        for epoch in range(self.args.epochs):
            ...
            current_valid_metric = np.mean([results_valid['mrr_lhs'], results_valid['mrr_rhs']])
            ...
            if current_valid_metric > best_valid_metric:
                ...
                # 保存当前最佳模型
                self.save_model()
                self.logger.info(f"Epoch {epoch}: New best valid metric {best_valid_metric}. Model saved.")
            else:
                patience_counter += 1
                self.logger.info(f"Epoch {epoch}: No improvement in valid metric. Patience counter: {patience_counter}")
            ...
            if patience_counter >= patience:
                ...
                results_test = self.evaluate(split='target', batch_size=self.args.test_batch_size, epoch=epoch)
                break
