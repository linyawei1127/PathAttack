...
class WN18RR(data.KnowledgeGraphDataset):
    ...
    def __init__(self, path, verbose=1):
        path = os.path.expanduser(path)
        if not os.path.exists(path):
            raise ValueError(f"Path '{path}' does not exist. Please provide a valid local dataset path.")
        self.path = path
        ...
        for file in txt_files:
            if not os.path.isfile(file):
                raise FileNotFoundError(f"Required file '{file}' is missing in the specified path.")
