...
#         path = 'data/fb15k237'
#         transductive_files = [
#             os.path.join(path, f"v1_train.txt"),
#             os.path.join(path, f"v1_valid.txt"),
#             os.path.join(path, f"v1_test.txt"),
#         ]
#
#         inductive_files = [
#             os.path.join(path, f"v1_ind_train.txt"),
#             os.path.join(path, f"v1_ind_valid.txt"),
#             os.path.join(path, f"v1_ind_test.txt"),
#         ]
#
#         # 检查文件是否存在
...
class FB15k237Inductive(InductiveKnowledgeGraphDataset):
    ...
    transductive_urls = [
        "https://github.com/kkteru/grail/tree/master/data/fb237_%s/train.txt",
        "https://github.com/kkteru/grail/tree/master/data/fb237_%s/valid.txt",
        "https://github.com/kkteru/grail/tree/master/data/fb237_%s/test.txt",
    ]
    ...
    def __init__(self, path, version="v1", verbose=1):
        ...
        for url in self.transductive_urls:
            url = url % version
            save_file = os.path.basename(url)  # 保持与实际文件名一致
            txt_file = os.path.join(path, save_file)
            if not os.path.exists(txt_file):
                raise FileNotFoundError(f"Transductive file not found: {txt_file}")
        ...
        for url in self.inductive_urls:
            ...
            txt_file = os.path.join(path, save_file)
            if not os.path.exists(txt_file):
                raise FileNotFoundError(f"Inductive file not found: {txt_file}")
